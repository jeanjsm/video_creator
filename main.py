...
def main():
    ...
    logging.basicConfig(filename='debug_video_creator.log', level=logging.DEBUG, format='%(asctime)s %(levelname)s %(message)s')
    ...
    # Capa (cover)
    parser.add_argument('--capa', help='Arquivo de imagem para capa (cover) sobre o vídeo')
    parser.add_argument('--capa_opacidade', type=float, default=1.0, help='Opacidade da capa (0.0 a 1.0, padrão=1.0)')
    parser.add_argument('--capa_tamanho', type=float, default=1.0, help='Tamanho relativo da capa (0.0 a 1.0, padrão=1.0)')
    parser.add_argument('--capa_posicao', choices=['centro', 'top_left', 'top_right', 'bottom_left', 'bottom_right'], default='centro', help='Posição da capa (padrão: centro)')
    ...
    parser.add_argument('--volume_musica', type=float, default=0.2, help='Volume da música de fundo (0.0 a 1.0, padrão=0.2)')
    parser.add_argument('--logo', help='Arquivo da logo (opcional)')
    parser.add_argument('--logo_posicao', choices=['top_left', 'top_right', 'bottom_left', 'bottom_right'], default='top_right', help='Posição da logo (padrão: top_left)')
    parser.add_argument('--logo_opacidade', type=float, default=1.0, help='Opacidade da logo (0.0 a 1.0, padrão=1.0)')
    parser.add_argument('--logo_tamanho', type=float, default=0.15, help='Tamanho relativo da logo (0.0 a 1.0, padrão=0.15)')
    ...
    # Overlays
    parser.add_argument('--overlay1', help='Arquivo de overlay 1 (imagem ou vídeo, opcional)')
    parser.add_argument('--overlay1_opacidade', type=float, default=0.4, help='Opacidade do overlay 1 (0.0 a 1.0, padrão=1.0)')
    parser.add_argument('--overlay2', help='Arquivo de overlay 2 (imagem ou vídeo, opcional)')
    parser.add_argument('--overlay2_opacidade', type=float, default=0.4, help='Opacidade do overlay 2 (0.0 a 1.0, padrão=1.0)')
