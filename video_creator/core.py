...
def _norm_pos(p):
    ...
    aliases = {
        'centro': 'center', 'meio': 'center', 'middle': 'center', 'c': 'center',
        'top': 'top_center', 'tc': 'top_center',
        'bottom': 'bottom_center', 'bc': 'bottom_center',
        'tl': 'top_left', 'tr': 'top_right', 'bl': 'bottom_left', 'br': 'bottom_right',
        'inferior_direita': 'bottom_right', 'direita_baixo': 'bottom_right',
    }
...
def _overlay_chain(last_video_map: str, src_label: str, x: str, y: str, out_label: str) -> str:
...
def build_audio_inputs(audio_path, background_music_path, dur_audio):
    ...
    if background_music_path:
        inputs += ['-stream_loop', '-1', '-t', str(max(1.0, dur_audio)), '-i', background_music_path]
    return inputs
...
def build_logo_args_and_filter(logo_path, logo_pos, logo_opacity, logo_scale, *,
                               logo_input_idx: int, base_video_idx: int = 0):
    if not logo_path:
    ...
    overlay = f'{_as_link(f"{base_video_idx}:v")}[logo]overlay={x}:{y}:format=auto[outv]'
...
def render_none(img, out_file, segment_duration, ffmpeg_path, encoder, framerate):
    ...
    return [
        ffmpeg_path, '-y', '-loop', '1', '-t', str(segment_duration),
        '-i', os.path.abspath(img), '-vf', vf, '-r', str(framerate),
        '-c:v', encoder, *get_encoder_flags(encoder, framerate),
        '-movflags', '+faststart', '-pix_fmt', 'yuv420p', '-an', out_file
    ]
...
def render_simplezoom(img, out_file, segment_duration, ffmpeg_path, encoder, framerate):
    ...
    return [
        ffmpeg_path, '-y', '-loop', '1', '-i', os.path.abspath(img),
        '-vf', vf, '-c:v', encoder, *get_encoder_flags(encoder, framerate),
        '-movflags', '+faststart', '-pix_fmt', 'yuv420p', '-an', '-frames:v', str(d_frames), out_file
    ]
...
def render_pendulo(img, out_file, segment_duration, ffmpeg_path, encoder, framerate, ffprobe_path=None):
    ...
    return [
        ...
        '-threads', '0', '-filter_threads', '0', '-an', out_file
    ]
...
def concat_with_transitions_singlepass(segment_files, saida, segment_duration,
                                       ffmpeg_path, encoder, tipo_transicao='fade',
                                       transition=None):
    ...
    if n == 1:
        ...
        for i in range(1, n):
            ...
            lines.append(f"{prev}[v{i}]xfade=transition={tipo_transicao}:duration={transition}:offset={off}[x{i}]")
            prev = f"[x{i}]"
    ...
    cmd = [
        ...
        '-pix_fmt', 'yuv420p', '-shortest', '-movflags', '+faststart', saida
    ]
...
def criar_video(
    ...
    if not transicao:
        ...
        if overlays:
            for ov in overlays:
                ...
                is_video = os.path.splitext(ov_path)[1].lower() in {'.mp4', '.mov', '.avi', '.mkv', '.webm'}
                if is_video:
                    overlay_inputs += ['-stream_loop', '-1', '-i', ov_path]
                else:
                    overlay_inputs += ['-i', ov_path]
                ov_idx = next_input_idx; next_input_idx += 1
            ...
            for chroma in overlay_chromas:
                ...
                if c_dur <= 0:
                    try:
                        ffprobe = _resolve_bin('ffprobe', ffprobe_path)
                        r = subprocess.run([
                            ffprobe, '-v', 'error', '-show_entries', 'format=duration', '-of', 'json', cpath
                        ], capture_output=True, text=True, check=True)
                        c_dur = float(json.loads(r.stdout)['format']['duration'])
                    except Exception:
                        c_dur = 0.0
                overlay_inputs += ['-i', cpath]
                c_idx = next_input_idx; next_input_idx += 1
        ...
        if capa:
            ...
            capa_idx = next_input_idx; next_input_idx += 1
        ...
        cmd += ['-c:v', encoder, *get_encoder_flags(encoder, framerate=30), '-pix_fmt', 'yuv420p']
        if filter_parts:
    ...
    # ===== COM TRANSIÇÃO =====
    pre_dir = os.path.join(os.path.dirname(os.path.abspath(saida)), 'pre_rendered_image_segments')
    ...
    for img in imagens_unicas:
        ...
        if efeito == 'fade': cmd_r = render_fade(img, out_file, segment_duration, ffmpeg_path, encoder, fr)
        elif efeito == 'zoom': cmd_r = render_zoom(img, out_file, segment_duration, ffmpeg_path, encoder, fr)
        elif efeito == 'simplezoom': cmd_r = render_simplezoom(img, out_file, segment_duration, ffmpeg_path, encoder, fr)
        elif efeito == 'pendulo': cmd_r = render_pendulo(img, out_file, segment_duration, ffmpeg_path, encoder, fr, ffprobe_path)
        else: cmd_r = render_none(img, out_file, segment_duration, ffmpeg_path, encoder, fr)
    ...
    concat_with_transitions_singlepass(segment_files, saida=temp_video,
                                       segment_duration=segment_duration,
                                       ffmpeg_path=ffmpeg_path, encoder=encoder,
                                       tipo_transicao=tipo_transicao, transition=None)
